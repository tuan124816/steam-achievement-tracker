...
def export_styled_excel(df, friends, out_path):
    ...
    Args:
        df (pd.DataFrame): DataFrame containing columns
            ["Achievement name", "Description", friend1, friend2, ...]
    ...
    # ====== Column layout ====== #
    ws.set_column(0, 0, 40)     # Achievement name
    ws.set_column(1, 1, 80)     # Description
    for i in range(2, len(cols)):
        ws.set_column(i, i, 14)
    ws.freeze_panes(1, 2)       # Freeze top row + first two columns
    ...
    for r, row in enumerate(df.itertuples(index=False, name=None), start=1):
        ...
        ws.write(r, 0, row[0], row_fmt[idx])    # Achievement name
        ws.write(r, 1, row[1], text_fmt[idx])   # Description
